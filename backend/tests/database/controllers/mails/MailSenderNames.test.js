	...
	describe('upsert', () => {
		...
		it('throws an error if record with a given "id" does not exist', async () => {
			try {
				...
				expect(message).toEqual('Could not find record in table mail_sender_names -> {}');
		...
		it('throws an error if record with given "name" already exists', async () => {
			try {
				...
				expect(message).toEqual('insert into "mail_sender_names" ("name") values ($1) returning "id" - duplicate key value violates unique constraint "mail_sender_names_name_unique"');
