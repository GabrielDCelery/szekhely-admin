    ...
    describe('findOrCreate', () => {
        ...
        it('throws an error if finds more than one records', async () => {
            try {
                ...
                return expect(message).toEqual('Found more than one records in table address_cities -> {"country_id":1}');
            }
        ...
        it('throws an error if trying to create a new city with impartial data', async () => {
            try {
                ...
                return expect(message).toEqual('insert into "address_cities" ("name") values ($1) returning "id" - null value in column "country_id" violates not-null constraint');
            }
        ...
        it('throws an error if trying to create a new city with non-existent country', async () => {
            try {
                ...
                return expect(message).toEqual('insert into "address_cities" ("country_id", "name") values ($1, $2) returning "id" - insert or update on table "address_cities" violates foreign key constraint "address_cities_country_id_foreign"');
            }
        ...
        it('throws an error if trying to find a country where the id does not exist', async () => {
            try {
                ...
                return expect(message).toEqual('Could not find record in table address_cities -> {"id":22}');
            }
